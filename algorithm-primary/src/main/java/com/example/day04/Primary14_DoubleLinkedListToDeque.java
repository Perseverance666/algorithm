 ...
 * @Date: 2022/9/9 17:25
 *
 * 用双链表结构实现双端队列（单链表不支持，无法从tail端弹出）
 *
 *  从head或tail弹出时，一定要将改变后的head.last或tail.next变为null，防止jvm无法删除，形成脏数据
 */
public class Primary14_DoubleLinkedListToDeque {
    ...
    public static class MyDeque<V> {
        ...
        public V pollHead() {
            ...
            ans = head.value;
            size--;
            if(head == tail){    //队列只有一个数据
                head = null;
                tail = null;
            }else{          //队列中超过一个数据
                head = head.next;
                head.last = null;   //防止形成脏数据
            }
            return ans;
        }
