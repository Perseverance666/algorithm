 ...
 * 消除字母
 * <p>
 * 题目：如果一个字符相邻的位置没有相同字符，那么这个位置的字符出现不能被消掉
 * 比如:"ab"，其中a和b都不能被消掉
...
public class Code03_DeleteAdjacentSameCharacter {
    ...
    public static int process(char[] str, int L, int R, boolean has) {
        ...
        int way1 = (all > 1 ? 0 : 1) + process(str, index, R, false);
        ...
        for (int split = index; split <= R; split++) {
            if (str[split] == str[L] && str[split] != str[split - 1]) {
                ...
                if (process(str, index, split - 1, false) == 0) {
                    ...
                    way2 = Math.min(way2, process(str, split, R, all > 0));
                }
            }
        }
    ...
    //消除L到R上字符，返回最少的剩余字符数量。has T表示L左边的字符与L的一样，has F表示不一样
    public static int process3(char[] str, int L, int R, boolean has, int[][][] dp) {
        if (L > R) {
            //越界了，返回0
            return 0;
        }
        ...
        if (L == R) {
            ...
            // index此时在第一个不是[L]字符的位置。
            //1、L~index自己消除，index~R再消除
            int way1 = (all > 1 ? 0 : 1) + process3(str, index, R, false, dp);
            int way2 = Integer.MAX_VALUE;
            for (int split = index; split <= R; split++) {
                if (str[split] == str[L] && str[split] != str[split - 1]) {
                    //找到后面第一个与L相同字符的位置split
                    if (process3(str, index, split - 1, false, dp) == 0) {
                        //2、如果index~split都能消除，再消除split~R上字符
