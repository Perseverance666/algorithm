...
题目：只由小写字母（a~z）组成的一批字符串，都放在字符类型的数组String[] arr中，如果其中某两个字符串所含有的字符种类完全一样，就将两个字符串算作一类，比如baacbba和bac就算作一类，返回arr中有多少类
...
题目：给定整数数组nums和目标值goal，需要从nums中选出一个子序列，使子序列元素总和最接近goal，也就是说如果子序列元素和为sum ，需要最小化绝对差abs(sum - goal)，返回 abs(sum - goal)可能的最小值。注意数组的子序列是通过移除原始数组中的某些元素（可能全部或无）而形成的数组。
...
题目：给定三个参数，二叉树的头节点head，树上某个节点target，正数K。从target开始，可以向上走或者向下走，返回与target的距离是K的所有节点
...
### 1、二分法模板。对应day06的2,3题

模板要取while( l < r)。假设初始时我们的二分区间为[l,r]，每次二分缩小区间时，如果左边界l要更新为l = mid ，此时我们就使用模板2，让mid = (l + r + 1) / 2，r = mid。否则while会陷入死循环。

如果左边界l要更新为l = mid + 1，此时我们就使用模板1，让mid = (l + r ) / 2，r = mid - 1。

二分的while循环的结束条件时 l >= r，所以在循环结束时 l 有可能会大于r（暂时没遇见过），此时可能越界，因此，基本上二分问题结果取 r 都不会翻车

#### 1）模板1

while(l < r)， [l,r]划分成[l,mid]和[mid+1,r]。mid = (l + r)/2。l = mid + 1。 r = mid

只要记住 l = mid + 1时， r = mid，mid = (l + r)/2
...
while(l < r)，[l,r]划分成[l,mid-1]和[mid,r]。mid = (l + r + 1)/2。l = mid。r = mid - 1

只要记住 l = mid 时， r = mid - 1，mid = (l + r +1)/2 ，防止死循环
