 ...
 * 目标和
 *
 * 题目：给定一个数组arr，你可以在每个数字之前决定+或者-但是必须所有数字都参与，再给定一个数target
 * 请问最后算出target的方法数
 *
...
public class Code07_TargetSum {
    ...
    public static int process2(int[] arr,int i,int rest,HashMap<Integer, HashMap<Integer, Integer>> dp){
        ...
        if(i == arr.length){
            ...
            //当前数添加+
            int p1 = process2(arr,i+1,rest-arr[i],dp);
            //当前数添加-
            int p2 = process2(arr,i+1,rest+arr[i],dp);
            ans = p1 + p2;
        }
        if(!dp.containsKey(i)){
